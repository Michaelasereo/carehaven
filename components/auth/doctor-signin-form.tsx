...
export function DoctorSignInForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const redirectUrl = `/auth/callback?next=${encodeURIComponent(redirectTo || '/doctor/dashboard')}`
      
      console.log('🔄 Redirecting to doctor dashboard:', redirectUrl)
      window.location.href = redirectUrl
      return

    } catch (err) {
      console.error('Unexpected error during doctor sign in:', err)
